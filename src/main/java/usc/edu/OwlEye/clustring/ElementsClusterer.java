...
public class ElementsClusterer {
    ...
    public ElementsClusterer(Node<DomNode> elementsForClustering) {
        ...
        ArrayList<Node<DomNode>> visibleNodes = new ArrayList<Node<DomNode>>();
        //only cluster visible elements

        Queue<Node<DomNode>> q = new LinkedList<>();
     //   OwlEye.getOriginalDefaultUI().getXMLTree();
        q.add(elementsForClustering);
    ...
    private void prepareElementsForClustering(List<Node<DomNode>> elementsForClustering) {
        wrappedElems = new ArrayList<ElementWrapper>();
        for (Node<DomNode> elm : elementsForClustering) {
            ElementWrapper elmWraped = new ElementWrapper(elm);
            wrappedElems.add(elmWraped);
            xpathToElementMap.put(elm.getData().getxPath(), elmWraped); // Ali: I copied that map here
        }
    ...
    public void printClusterResults() {
        ...
        for (Cluster<ElementWrapper> cluster : clusterResults) {
            ...
            for (ElementWrapper element : cluster.getPoints()) {
                System.out.println("\t" + element.getDomNode().getData().getxPath());
            }

        }
    }

    public ArrayList<ArrayList<Node<DomNode>>> getClustringResultsDomNodes() {
        ArrayList<ArrayList<Node<DomNode>>> domNodesClusters = new ArrayList<ArrayList<Node<DomNode>>>();
        for (Cluster<ElementWrapper> cluster : clusterResults) {
            ArrayList<Node<DomNode>> DomNodesCluster = new ArrayList<Node<DomNode>>();
    ...
    private ArrayList<String> getXpathCluster(Cluster<ElementWrapper> cluster) {
        ...
        for (ElementWrapper element : cluster.getPoints()) {
            String xpath = element.getDomNode().getData().getxPath();
