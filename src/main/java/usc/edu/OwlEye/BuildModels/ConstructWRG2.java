...
public class ConstructWRG2 {

    protected static ConstructWRG2 instance = null;


    private ConstructWRG2(String subjectID) {
    }

    public static ConstructWRG2 getInstance() {

        if (instance == null)
            instance = new ConstructWRG2(null);  // do whatever else you need to here
    ...
    private void createPropertyWRG(WRG wrg, XMLUtils xmlTree, String property) {
        ...
        TreeMap<String, List<WRGNode>>  currPropertyWRG = wrg.getDependentNodesMap().get(property);
...
//            childrenWithWeight.put(ConstraintHorizontalWeight.propertyName, new ArrayList<Node<DomNode>>());
            ...
            if (childrenNodes != null) {
                ...
                if(childrenWithWeight.size()>0){
                    // parent to Child relationships
                    ArrayList currentNodeDependentNode=new ArrayList<WRGNode>();

                    for (Node<DomNode> node : childrenWithWeight.keySet()
                    ) {
                        double currNodeVal=childrenWithWeight.get(node);
                        double ratio=currNodeVal/parentTotal;
                        Property weightPropertyObj;
                        if (foundWeightPropertyForChildren.equalsIgnoreCase(Weight.propertyName)){
                            weightPropertyObj=new Weight();
                            String val=String.valueOf(currNodeVal);
                            weightPropertyObj.setCurrentDynamicVal(val);
                            weightPropertyObj.setCurrentStaticVal(val);
                        }
                        else if(foundWeightPropertyForChildren.equalsIgnoreCase(ConstraintHorizontalWeight.propertyName)){
                            weightPropertyObj= new ConstraintHorizontalWeight();

                        }
                        else if (foundWeightPropertyForChildren.equalsIgnoreCase(ConstraintVerticalWeight.propertyName)){
                            weightPropertyObj= new ConstraintVerticalWeight();
                        ...
                        WRGNode wrgNode = new WRGNode(node.getData().getxPath(),node,foundWeightPropertyForChildren,ratio,currNodeVal,parentTotal,parentNode.getData().getxPath(),weightPropertyObj);
                        currentNodeDependentNode.add(wrgNode);

                    }
                    currPropertyWRG.put(parentNode.getData().getxPath(),currentNodeDependentNode);
