...
import usc.edu.SALEM.segmentation.IntraSegmentEdge;
...
public class FitnessFunction {
    ...
    private double A11yHeuristicsScore = Double.MIN_VALUE;
    ...
    public void setA11yHeuristicsScore(double a11yHeuristicsScore) {
        this.A11yHeuristicsScore = a11yHeuristicsScore;
    }
    ...
    public double getA11yHeuristicsScore() {
        return A11yHeuristicsScore;
    }
...
//            DomNode currMergedNode = node.getData();
    ...
    public double calculateA11yHeuristicsScore(String chromosomeIdentifier, Node<DomNode> newLayoutRoot) {
        AccessibilityScannerResults chromosomeIssues = SALEM.getDetectionToolResults().get(chromosomeIdentifier);
//        double usabilityScore = chromosomeIssues.calculateAccessibilityScore(newLayoutRoot);
        double[] usabilityScore;
        // We are no longer calculating the score based on the accessibility scanner so we do not need this if
...
//        double noIssuesScore = numberOfIssues / noTT;
        ...
        double originalTTNO = SALEM.getCurrentActivityOriginalNoOfTT();
...
//        }
        if (calculateInitialArea) {
            ...
            SALEM.totalRecommendedArea = totalRecommendedArea;
        }
...
//                Constants.RECOMMENDED_AREA_FOR_TOUCH_TARGET);
        double missingSizeScore = totalcurrentArea / SALEM.totalRecommendedArea;
...
//        double score=noIssuesScorePercentage+missingSizeScoreNumber;
    ...
    public double[] calculateAccessibilityScore_beforeASE_Version(Node<DomNode> newLayoutRoot) {
...
//        double score=noIssuesScorePercentage+missingSizeScoreNumber;
    ...
    private double[] calculateIntraSegmentAestheticScore(SegmentModel updatedSegmentModel) {
        ...
        for (Segment originalIntrasegmentModel : SALEM.getOriginalUISegments()) {
    ...
    public void calculateFitnessScore(GAChromosome chromosome, String newLayoutFolder) {
        ...
        if (newLayoutFolder == null) {  // We could not read the layout so no point of calculating UI. we set as worst
            ...
            A11yHeuristicsScore = Double.MIN_VALUE;
            spacingScore = Double.MIN_VALUE;
            aestheticScore = 2;
        } else {

            String newLayout = newLayoutFolder + SALEM.getCurrentActivityName() + ".xml";
            ...
            A11yHeuristicsScore = calculateA11yHeuristicsScore(chromosomeIdentifier, newLayoutRoot);
            System.out.println("Accessibility Score for chromosome ID: " + chromosomeIdentifier + " is: " + A11yHeuristicsScore);
...
//            }
            ...
            // (2) Calculate the Aesthetic score (Relative Positioning and Alignments Objective) 
            ...
            if (Constants.SPACE_CALCULATION_APPROACH.equalsIgnoreCase("ALL")) {
                ...
                //calculate space between segments only
        ...
        double distanceA11yHeuristicsScore = Constants.EXPECTED_USABILITY_SCORE - A11yHeuristicsScore;
        distanceA11yHeuristicsScore = Math.abs(distanceA11yHeuristicsScore);

        double distanceAestheticScore = aestheticScore - Constants.EXPECTED_AESTHETIC_SCORE;


        // Test with sigmoid --- after ase test
...
//                (Constants.WEIGHT4 * Math.pow(distanceSpacingScore, Constants.EXP2));
        fitnessScore = (Constants.WEIGHT1 * Math.pow(distanceA11yHeuristicsScore, Constants.EXP1)) +
                (Constants.WEIGHT2 * Math.pow(distanceAestheticScore, Constants.EXP2)) + (Constants.WEIGHT5 * Math.pow(distanceSizeChangeScore, Constants.EXP2)) +
                (Constants.WEIGHT4 * Math.pow(distanceSpacingScore, Constants.EXP2));

        fitnessScoreBreakdown = "Accessibility = " + A11yHeuristicsScore + ", Asethtic = " + aestheticScore +
                ", Spacing = " + spacingScore + ", sizeChange = " + sizeChangeScore + " =>=> " +
                Constants.WEIGHT1 + "* (" + distanceA11yHeuristicsScore + "^" + Constants.EXP1 + ") + "
                ...
                + "\nIssues Percentage: " + this.getNoIssuesPercentage() + " ||| No Issues: " + noIssuesInChromosome + "||| existingTT/originalTT: " + this.getCurrent_no_TT() + "/" + SALEM.getCurrentActivityOriginalNoOfTT();
...
//            spacingScore = 100 - (score * 100);
    ...
    private double calculateSpacingScore(String chromosomeIdentifier) {
        ...
        /*** (1) get all elements that are close to each other ***/
        double totalRecommendedSpacing = 0;
        for (Segment seg : SALEM.getOriginalUISegments()
        ) {
...
//            spacingScore = 100 - (score * 100);
    ...
    public double calculateAestheticScore(Node<DomNode> newLayoutRoot) {
        ...
        if (Constants.DISTORTION_APPROACH.equalsIgnoreCase("usc/edu/SALEM")) {
            // get updated segment model and violated edges
            Util.updateLayoutWithNewValues(newLayoutRoot); // copied ti from Util class
            Node<DomNode> root = XMLUtils.getRoot();
            SegmentModel updatedSegmentModel = SALEM.getOriginalUISegmentModel().getUpdatedSegmentModel();
            SegmentModel original = SALEM.getOriginalUISegmentModel();
            ...
            List<InterSegmentEdge> interSegmentViolatedEdges = SALEM.getOriginalUISegmentModel().compareToSegmentModel(updatedSegmentModel);
            ...
            double possibleTotalViolatedScore = SALEM.getOriginalUISegmentModel().getEdges().size() * sumOfLabelWeights;
            ...
            aestheticScoreBreakdown = aestheticScoreBreakdown + " => " + interSegmentViolatedScoreBreakdown + " / "
                    + "(" + SALEM.getOriginalUISegmentModel().getEdges().size() + "*" + sumOfLabelWeights + ")";
        }
        else if (Constants.DISTORTION_APPROACH.equalsIgnoreCase("if")) {
